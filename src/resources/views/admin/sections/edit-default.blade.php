  ...
  <div class="h-full w-full bg-white rounded-xl shadow-lg p-10">
    ...
    <form action="{{ route('admin.sections.update', $section) }}" method="POST" enctype="multipart/form-data"
      ...
      <div class="space-y-2">
        ...
        <input type="url" id="link" name="link" value="{{ old('link', $section->link) }}"
      ...
      <div class="space-y-2">
        <label for="is_reversed" class="block text-xl font-semibold text-gray-700">
          Отзеркалить секцию <span class="text-red-500">*</span>
        </label>
        <select id="is_reversed" name="is_reversed" required
          ...
          <option value="" disabled {{ old('is_reversed', $section->is_reversed) === null ? 'selected' : '' }}>
            Выберите опцию
          </option>
          <option value="0" {{ old('is_reversed', $section->is_reversed) == 0 ? 'selected' : '' }}>
            Нет
          </option>
          <option value="1" {{ old('is_reversed', $section->is_reversed) == 1 ? 'selected' : '' }}>
            Да
          </option>
        </select>
        @error('is_reversed')
